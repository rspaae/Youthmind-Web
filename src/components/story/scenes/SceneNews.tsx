...
export const SceneNews: React.FC = () => {
  ...
  const featuredItem = newsList.find((i) => i.isFeatured);
  const regularItems = filteredItems.filter((i) => !i.isFeatured);

  return (
    <section id="news" className="py-20 sm:py-32 bg-ym-page text-ym-body border-t border-ym relative overflow-hidden transition-colors duration-300">
      ...
      <Container>
        <div className="max-w-6xl mx-auto space-y-12">
          {/* Header Editorial */}
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6 border-b border-ym">
            <div className="space-y-2 max-w-2xl">
              <div className="inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-teal-900 dark:text-teal-200 bg-[#4BA0A4]/15 px-3 py-1 rounded-full border border-[#4BA0A4]/30">
              ...
              <p className="text-sm sm:text-base text-ym-body font-normal leading-relaxed">
                  ...
                  <button
                    ...
                    className={`px-3.5 py-1.5 rounded-lg text-xs font-semibold transition-all duration-200 border focus:outline-none ${
                      isActive
                        ...
                        : "bg-ym-subtle text-ym-body border-ym hover:text-ym-heading hover:bg-ym-card"
                    }`}
                  >
                    {tab.label}
                  </button>
          ...
          {(activeCategory === "all" || activeCategory === "prestasi") && featuredItem && (
            <motion.div
              ...
              className="relative bg-ym-card border border-amber-400/50 rounded-3xl p-6 sm:p-10 shadow-xl overflow-hidden group hover:border-amber-400 transition-all"
            >
              ...
              <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-6 items-center">
                <div className="lg:col-span-8 space-y-4">
                  <div className="flex items-center gap-3 flex-wrap">
                    ...
                    <span className="text-xs text-ym-muted font-medium">
                      {featuredItem.date}
                    </span>
                  </div>

                  <h3 className="text-2xl sm:text-4xl font-extrabold text-ym-heading leading-tight group-hover:text-ym-accent transition-colors">
                    {featuredItem.title}
                  </h3>
                ...
                <div className="lg:col-span-4 flex lg:justify-end">
                  <div className="w-full lg:w-auto p-6 rounded-2xl bg-ym-subtle border border-amber-400/30 text-center space-y-2 shadow-md">
                    ...
                    <span className="text-[11px] text-ym-muted block leading-snug">
            ...
            <AnimatePresence mode="popLayout">
              {regularItems.map((item) => (
                <motion.div
                  ...
                  className="bg-ym-card border border-ym rounded-2xl p-6 flex flex-col justify-between hover:border-[#4BA0A4] transition-all duration-200 shadow-sm hover:shadow-md group"
                >
                  <div className="space-y-3">
                    <div className="flex items-center justify-between gap-2">
                      <span className="text-[11px] font-semibold text-teal-900 dark:text-teal-200 bg-[#4BA0A4]/15 px-2.5 py-0.5 rounded-md border border-[#4BA0A4]/30">
                        {item.tag}
                      </span>
                      <span className="text-[11px] text-ym-muted font-medium">
                        {item.date}
                      </span>
                    </div>

                    <h3 className="text-base font-bold text-ym-heading group-hover:text-ym-accent transition-colors leading-snug">
                      {item.title}
                    </h3>

                    <p className="text-xs text-ym-body font-normal leading-relaxed">
                      {item.summary}
                    </p>
                  </div>

                  <div className="pt-4 mt-4 border-t border-ym flex items-center justify-between text-xs text-teal-800 dark:text-teal-300 font-semibold">
                    <span>{item.publisher}</span>
                    <ArrowUpRight className="w-3.5 h-3.5 text-[#4BA0A4] group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-all" />
                  </div>
                </motion.div>
              ))}
            </AnimatePresence>
          ...
          <div className="bg-ym-card border border-ym rounded-2xl p-6 sm:p-8 flex flex-col sm:flex-row items-center justify-between gap-6 shadow-sm">
            <div className="space-y-1 text-center sm:text-left">
              <h4 className="text-base sm:text-lg font-bold text-ym-heading">
                Perlu Logo, Foto Produk, atau Berkas Resmi YouthMind?
              </h4>
              <p className="text-xs text-ym-muted max-w-xl leading-relaxed">
            ...
            <div className="flex items-center gap-3 w-full sm:w-auto">
              ...
              <a
                ...
                className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-ym-subtle hover:bg-ym-card text-ym-heading font-semibold text-xs border border-ym transition-all shadow-sm"
              >
                <span>Instagram</span>
